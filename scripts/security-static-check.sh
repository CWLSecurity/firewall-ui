...
if ! command -v rg >/dev/null 2>&1; then
  echo "[ui-security-static][fail] ripgrep (rg) is required" >&2
  exit 1
fi

echo "[ui-security-static] scanning for leaked credentials and keys"

if rg -n -S "(ghp_[A-Za-z0-9]{30,}|cfat_[A-Za-z0-9]{20,}|-----BEGIN (RSA|OPENSSH|EC) PRIVATE KEY-----)" \
...
if rg -n -S "(RELAYER_PRIVATE_KEY|DEPLOYER_PK|PRIVATE_KEY|BOT_DEPLOY_SSH_KEY|GITHUB_TOKEN|CLOUDFLARE_API_TOKEN)\\s*[:=]\\s*['\\\"]?0x[a-fA-F0-9]{64}" \
  "${ROOT_DIR}/src" "${ROOT_DIR}/server" "${ROOT_DIR}/scripts" "${ROOT_DIR}/public"; then
